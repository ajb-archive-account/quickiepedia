...
func TestRouter(t *testing.T) {
	...
	if resp.StatusCode != http.StatusOK {
		t.Errorf("Response should be status 200, got %d (OK)", resp.StatusCode)
	}
...
func TestStaticFileServer(t *testing.T) {
	...
	// `GET /assets/` route to get the index.html file response
	resp, err := http.Get(mockServer.URL + "/assets/")
	if err != nil {
		t.Fatal(err)
	}

	// Log for status 200 (ok)
	if resp.StatusCode != http.StatusOK {
		t.Errorf("Response should be status 200, got %d", resp.StatusCode)
	}
